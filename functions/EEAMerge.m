...
%
% ISSUES -> negative value da portare a NAN. Tenere traccia delle
% osservazioni rimosse ritornando in output le tabella: Negative
...
% pollutant(ismissing(pollutant(:,1))) = [];
tStart = min(DataTable.Time);
%tStart.TimeZone = DataTable.Properties.RowTimes(1).TimeZone;

tEnd = max(DataTable.Time);
%tEnd.TimeZone = Properties.RowTimes(1).TimeZone;
...
% todo
% sort metadata on AirQualityStation
